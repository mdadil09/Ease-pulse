...
import PassworldConfDialog from "./PassworldConfDialog";

const Password = ({ patient, setIsLoading }: any) => {
  ...
  const onSubmit = async (values: z.infer<typeof PasswordFormValidation>) => {
    ...
    try {
      const updatedPasswordData = await updatePassword({ email, newPassword });
